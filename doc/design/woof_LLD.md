...
**Localization (i18n)**: Static UI chrome is localized with [Paraglide JS](https://inlang.com/m/gerre34r); supported locales are `en` (base), `en-GB`, `fr`, `de`, `es`, `zh` (Simplified), and `ja`. Message catalogues live in `gallery/messages/{locale}.json` (the source of truth); Paraglide compiles them to `gallery/src/paraglide/` (gitignored, regenerated by the Vite plugin on every dev/build/test, and excluded from eslint). Non-obvious design points:

- **Locale is host-driven, in-memory.** The active locale is resolved from `HostContext.locale` — via the same `getHostContext()` / `onhostcontextchanged` channel used for theme — falling back to `navigator.language`, then the base locale. `lib/locale.js` resolves an arbitrary BCP-47 tag (exact → primary subtag → base, so `fr-FR`→`fr`, `zh-Hans-CN`→`zh`, while `en-GB` stays distinct from `en`) and calls Paraglide's `setLocale(tag, { reload: false })`. Paraglide is compiled with the **`globalVariable` strategy** specifically because the gallery is a sandboxed iframe: the default cookie/URL strategies would reload or navigate the frame to switch language, which is not available here. Host-pushed locale changes re-render the UI live.
- **`en-GB` is a full catalogue identical to `en`.** None of the current strings have a US/GB spelling divergence, but Paraglide requires every locale to define each key. It exists so a British host locale resolves distinctly and as the seam for future GB-specific wording. A `messages` completeness test enforces that every locale defines the base key set.
- **Plurals use paired `_one`/`_other` keys** (e.g. `status_photos_one` / `status_photos_other`) selected in JS, rather than ICU plural matchers — simpler and sufficient for the six locales (zh/ja define both variants identically). Dates/numbers are formatted with `Intl` using the active locale via `getLocale()`.
- **Only UI chrome is localized.** Photo metadata, Wally's `querySummary`, and model-facing text — the indexing summary markdown and the `sendMessage` triggers in `IndexingProgress` — are assistant/data content and are intentionally left in English.
